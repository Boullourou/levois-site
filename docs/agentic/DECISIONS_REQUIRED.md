...
### 1.1 État courant au 2026-08-19

- `decided` : D-001, D-002, D-003, D-004, D-005, D-006, D-010, D-011, D-012, D-015, D-016 et D-017 ;
- `open` : D-007, D-008, D-009, D-013, D-014 et D-018, ainsi que D-019 à D-056 ;
- aucune autre décision n'est déduite de la validation de l'architecture cible ;
- une décision `open` reste un verrou : un garde-fou provisoire peut permettre des fixtures fictives ou le shadow mode, jamais une activation avec données réelles lorsque la décision est requise.

### 1.2 Journal des décisions P0 validées

| ID / statut | Date / décideur | Décision enregistrée | Justification | Périmètre | Conditions de réexamen |
|---|---|---|---|---|---|
| D-001 `decided` | 2026-08-19 / Mouaad | La North Star reste une conversation humaine qualifiée, comptée uniquement selon la définition opérationnelle de `KPI_SYSTEM.md` après un échange réel. `conversation_ready` reste un état amont, jamais le résultat. | Empêche l'optimisation des formulaires, clics, contenus, tâches ou annonces analysées au détriment de la relation utile. | Toute mesure, priorisation, acquisition et briefing LEVOIS. | Revoir si la définition produit des faux positifs, ne distingue plus préparation et échange réel, ou si le modèle de relation humaine change. |
| D-002 `decided` | 2026-08-19 / Mouaad | L'architecture cible est hybride. La première tranche est cloud-only et hybrid-ready ; un runtime local ou isolé n'est ajouté qu'après besoin mesuré. | Offre accès cockpit, disponibilité et contrôle central sans introduire prématurément un transport local complexe. | Control plane minimal, `COS-01` et `OPS-01` de la première tranche. | Revoir si un besoin de fichiers locaux/Obsidian, de confidentialité, de disponibilité ou de coût ne peut pas être satisfait dans la tranche cloud. |
| D-003 `decided` | 2026-08-19 / Mouaad | D1 reste l'unique source de vérité opérationnelle. Missions, journaux, agents, GitHub et Obsidian ne deviennent jamais une base métier concurrente. | Préserve une autorité reconstructible et évite les divergences silencieuses entre cockpit et agents. | Toute donnée métier, projection, mission et audit. | Revoir uniquement sur limite D1 mesurée ou obligation réglementaire documentée ; jamais sur préférence de fournisseur. |
| D-004 `decided` | 2026-08-19 / Mouaad | Les neuf rôles logiques sont l'organigramme cible. Seuls `COS-01` minimal et `OPS-01` sont activables dans la première tranche ; les sept autres restent désactivés. | Réduit coordination, coûts et surface d'accès pour 5 à 20 dossiers. | Catalogue d'agents, permissions, roadmap et fixtures A0/A1. | Revoir après preuve de valeur de la tranche, charge réelle ou séparation de permissions justifiant un nouveau runtime. |
| D-005 `decided` | 2026-08-19 / Mouaad | Toutes les capacités commencent à L0. Aucune action externe autonome ni mutation automatique d'une vérité métier ; paiement, offre, mandat, matching final, publication sensible, consentement, fusion, suppression et action juridique/financière restent interdits. | Maintient responsabilité professionnelle, confiance et test d'autorité. | Tous agents, missions, connecteurs et niveaux d'autonomie. | Toute évolution exige une décision distincte, des tests d'autorité, un périmètre réversible et une revue sécurité/conformité ; les interdictions professionnelles ne sont pas levées implicitement. |
| D-006 `decided` | 2026-08-19 / Mouaad | Mouaad est l'unique approbateur V1. Son silence, son absence ou l'expiration d'une demande ne vaut jamais approbation ; aucun délégataire n'est prévu. | Ferme les chemins d'engagement implicite et rend l'autorité vérifiable. | Approbations, réactivations, exceptions, budgets et actions sensibles. | Revoir avant tout second utilisateur approbateur, avec nouveau modèle RBAC, délégation explicite et audit. |
| D-010 `decided` | 2026-08-19 / Mouaad | L'enveloppe conceptuelle versionnée de `EVENT_CATALOG.md` est le contrat canonique : provenance, agrégat/version, corrélation/causalité, idempotence, date, classification et payload minimal par type ; aucun payload universel. | Permet routage et rejeu contrôlés sans transformer le journal en seconde vérité métier. | Événements lus ou référencés par le control plane. | Revoir par nouvelle version avant tout nouveau producteur, consommateur ou champ d'autorité ; compatibilité et tests contractuels obligatoires. |
| D-011 `decided` | 2026-08-19 / Mouaad | Le cycle canonique est `draft → planned → assigned → running → waiting_input / waiting_approval → completed / failed / cancelled`. Les états terminaux ne rouvrent jamais ; une reprise crée une mission corrélée distincte. | Évite les reprises implicites, doubles exécutions et statuts inventés. | Missions et tentatives de la première tranche puis agents futurs. | Revoir si une primitive de queue/concurrence mesurée exige une transition supplémentaire ; toute modification reste versionnée et migrable. |
| D-012 `decided` | 2026-08-19 / Mouaad | Une approbation est liée à l'action, la cible, l'artefact complet, son hash, ses sources/versions, son coût et son expiration. Elle est consommée une fois après revalidation, puis Mouaad déclenche séparément l'action. | Empêche réutilisation, changement de contexte et confusion entre approbation et exécution. | Modèle d'approbation cible ; la Phase A1 sans action externe n'en active que la structure nécessaire à l'audit éventuel. | Revoir avant nouveau canal, nouvelle catégorie d'action, approbation groupée ou ajout d'un approbateur. |
| D-015 `decided` | 2026-08-19 / Mouaad | Mouaad possède seul le kill switch et l'autorité de réactivation. La première tranche prévoit au minimum les portées globale, agent et capability ; activation fermée par défaut et missions affectées annulées avec raison. | Assure un arrêt immédiat sans bloquer le cockpit manuel ni rouvrir silencieusement une mission. | `COS-01`, `OPS-01`, leurs missions et capacités A1. | Revoir les portées techniques lors d'un nouveau runtime/connecteur ou après incident ; jamais l'autorité humaine exclusive sans décision RBAC distincte. |
| D-016 `decided` | 2026-08-19 / Mouaad | `OPS-01` ne lit que les vues minimales nécessaires : IDs opaques, type/statut actif, prochaine action, tâches/échéances, promesses, statut de triage disponible et flags/échéances TIM utiles. Coordonnées, textes bruts et montants inutiles sont exclus. | Rend les détections déterministes possibles avec une surface PII minimale. | Les sept détections shadow explicitement listées pour `OPS-01`. | Revoir champ par champ avant une nouvelle règle ; une extension de vue exige preuve de nécessité et revue de minimisation. |
| D-017 `decided` | 2026-08-19 / Mouaad | Le briefing quotidien vise 3 à 7 priorités, n'en contient jamais plus de 7 et ne complète jamais artificiellement une journée qui n'en comporte que 0 à 2. Les éléments sont dédupliqués, ordonnés et expliqués par règle/source/date, avec action humaine suggérée. Aucun inventaire complet ni message externe. | Le briefing doit réduire la reconstitution mentale plutôt que créer une nouvelle boîte de réception. | Sortie de `COS-01` à partir des anomalies `OPS-01`. | Revoir l'ordre et les règles après shadow mode si une urgence est masquée ou le bruit trop élevé ; la borne de 7 ne s'élargit pas implicitement. |
...
| D-011 | Valider le cycle de mission et les transitions | Évite les états implicites | Cycle proposé ; reprise après état terminal = nouvelle mission corrélée | Scénarios de panne fictifs | Control plane |
...
### 2.1 Garde-fous provisoires pour les décisions P0 encore `open`

Ces mesures sont des **defaults de construction sur fixtures fictives**, pas des décisions métier, juridiques ou de production. Elles autorisent la préparation et les tests A1 ; elles interdisent l'activation avec données réelles tant que la décision correspondante reste `open`.

| ID | Recommandation opérationnelle provisoire A0/A1 | Verrou d'activation réelle |
|---|---|---|
| D-007 | Utiliser exclusivement des fixtures marquées synthétiques, sans modèle ni donnée client. Une classification absente ou inconnue produit un refus fermé ; ne pas figer de nouvel enum de production dans A1. | Aucune donnée réelle, aucun fournisseur de modèle et aucune vue non fictive avant validation des catégories et interdictions définitives. |
| D-008 | Nettoyer les fixtures avec l'environnement de test et conserver seulement les preuves nécessaires à la durée du test ; aucun TTL légal ou opérationnel n'est déduit. | Aucune persistance agentique réelle tant qu'une politique de rétention par finalité n'est pas décidée. |
| D-009 | Ne construire ni export ni effacement dans A1. Les fixtures restent jetables et aucune copie externe n'est produite. | Aucun endpoint, capability ou workflow réel d'export/effacement avant procédure définitive validée. |
| D-013 | N'autoriser aucun connecteur payant ni modèle. Utiliser un compteur logique de travail ; un budget absent refuse l'admission et le plafond monétaire externe provisoire reste zéro. | Aucun coût externe ou dépense réelle avant montant, devise, owner et cadence de revue décidés. |
| D-014 | Valeur par défaut shadow : zéro retry automatique ; timeout fourni explicitement par le scénario fictif, résultat tardif rejeté. L'absence de configuration échoue fermée. | Aucune mission sur données réelles avant valeurs par type, règles de retry et seuils de circuit breaker décidés. |
| D-018 | Vérifier les 15 critères fonctionnels A0/A1 et mesurer sans déclarer de seuil de go/no-go. `BUY-01` et toute hausse d'autonomie restent désactivés. | Aucun passage à `BUY-01`, L1+ ou données réelles avant baseline et seuils de réussite validés par Mouaad. |
